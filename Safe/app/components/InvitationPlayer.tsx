...
const submitRSVP = async () => {
  ...
  const cleanedGuests = guestNames.map((g) => g.trim()).filter(Boolean);
...
<div className="space-y-2">
  <div className="text-sm text-white/80">Guest names</div>

  <div className="max-h-40 overflow-y-auto space-y-2 pr-1">
    {guestNames.map((name, i) => (
      <div key={i} className="flex gap-2">
        <input
          className="flex-1 bg-black/40 border border-white/30 rounded px-3 py-2"
        ...
        <button
          ...
          onClick={() => {
            const arr = guestNames.filter((_, idx) => idx !== i);
            setGuestNames(arr.length ? arr : [""]);
          }}
        >
          ✕
        </button>
    ...
    {safeEvent.gallery.length === 0 ? (
      ...
      <>
        ...
        <button
          ...
          className="absolute left-3 bg-black/50 w-10 h-10 rounded-full text-xl"
        >
          ‹
        </button>
        ...
        <button
          ...
          className="absolute right-3 bg-black/50 w-10 h-10 rounded-full text-xl"
        >
          ›
        </button>
      </>
