...
const stats = useMemo(() => {
  ...
  const attendingPeople = rsvps
    ...
    .reduce((sum, r) => {
      const guests = Number(r.guest_count) || 0; // make sure it's a number
      return sum + 1 + guests; // 1 = the inviter + guests
    }, 0);
...
const deleteRSVP = async (id: string) => {
  ...
  const exportCSV = () => {
    ...
    const headers = ["Name", "Attending", "People Coming", "Note", "Date"];

const rows = rsvps.map((r) => [
  r.main_name,
  r.attending ? "Yes" : "No",
  r.attending ? 1 + (Number(r.guest_count) || 0) : 0,
  r.note || "",
...
const data = rsvps.map((r) => ({
  ...
  "People Coming": r.attending ? 1 + (Number(r.guest_count) || 0) : 0,
  Note: r.note || "",
...
<td className="font-semibold">
  {r.attending ? 1 + (Number(r.guest_count) || 0) : "-"}
</td>
